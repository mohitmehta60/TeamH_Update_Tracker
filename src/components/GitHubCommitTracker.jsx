...
import { ChevronDown, ChevronRight, ExternalLink, GitCommit, Clock, User, Hash } from 'lucide-react';
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          throw new Error('Rate limit exceeded. Add VITE_GITHUB_TOKEN to .env for higher limits');
        } else {
      ...
      setCommits(prev => ({ ...prev, [repoKey]: commitsData }));
    } catch (error) {
      setErrors(prev => ({ 
        ...prev, 
        [repoKey]: error instanceof Error ? error.message : 'Failed to fetch commits' 
      }));
    } finally {
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                      ...
                      <div className="p-6">
                        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                          <div className="flex items-center space-x-2">
                            ...
                            <p className="text-red-700 font-medium">Error loading commits</p>
                          </div>
                    ...
                    {!isLoading && !error && repoCommits.length > 0 && (
                      <div className="p-6 space-y-4">
                        ...
                        {repoCommits.map((commit, commitIndex) => {
                          ...
                          const hasLongMessage = fullMessage.length > truncatedMessage.length;
                                  ...
                                  <div className="mb-3">
                                    <p 
                                      className={`text-gray-800 font-medium leading-relaxed ${
                                        hasLongMessage ? 'cursor-pointer hover:text-gray-600' : ''
                                      }`}
                                      onClick={() => hasLongMessage && toggleCommitExpansion(commit.sha)}
                                    >
                                      {isExpanded ? fullMessage : truncatedMessage}
                                      {hasLongMessage && (
                    ...
                    {!isLoading && !error && repoCommits.length === 0 && (
                      <div className="p-8">
                        <div className="text-center text-gray-500">
                          ...
                          <p className="text-sm mt-1">This repository might be empty or private</p>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        {repositories.length === 0 && (
          <div className="p-12 text-center text-gray-500">
            ...
            <p className="text-sm mt-2">Add repositories to track their commits</p>
          </div>
        )}
      </div>
    </div>
